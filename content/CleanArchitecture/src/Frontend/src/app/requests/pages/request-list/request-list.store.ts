...
import { PaginatedListOfTodoItemBriefDto, TodoItemsClient } from "../../../_shared/services/api.service";
...
export const RequestListStore = signalStore(
  ...
  withMethods((store, client = inject(TodoItemsClient)) => ({
    async loadItems(listId: number, pageNumber: number, pageSize: number) {
      const result = await firstValueFrom(client.getTodoItemsWithPagination(listId, pageNumber, pageSize));
